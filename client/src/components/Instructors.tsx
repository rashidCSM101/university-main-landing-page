        ...
        {loading ? (
          ...
          <div ref={cardsRef} className="grid md:grid-cols-2 lg:grid-cols-4 gap-7">
            {scientists.map((sc) => (
              <div
                key={sc.id}
                className="group relative bg-white p-2.5 rounded-[30px] border border-gray-200/90 shadow-lg hover:shadow-2xl transition-all duration-500 flex flex-col justify-between hover:-translate-y-2"
              >
                    ...
                    <img
                      ...
                      onError={(e) => {
                        ...
                        if (e.currentTarget.parentElement) {
                          e.currentTarget.parentElement.innerHTML = `<div class="w-full h-full bg-gradient-to-br from-[#48b302] to-teal-900 text-white font-bold text-4xl flex items-center justify-center">${getInitials(sc.name)}</div>`;
                        }
                      }}
